...
public class LogInController {
	...
	public HashMap<String,String> checkUsernameCredentials(@RequestBody User user) {
		...
		if(parentRepo.getSpecificParent(user.getUsername().toLowerCase(),user.getPassword())!=null) {
			id = (parentRepo.getSpecificParent(user.getUsername().toLowerCase(),user.getPassword())).getId();
			System.out.print(id);
			hash.put("ID", id);
			hash.put("flag","4" );
			return hash;
		}
		if(adminRepo.getSpecificAdmin(user.getUsername().toLowerCase(),user.getPassword())!=null) {
			id = (adminRepo.getSpecificAdmin(user.getUsername().toLowerCase(),user.getPassword())).getId();
			System.out.print(id);
			hash.put("ID", id);
			hash.put("flag","1" );
			return hash;
